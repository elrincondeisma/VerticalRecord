#!/bin/sh
# Monta el DMG de distribución: MiniOBS.app + enlace a /Applications.
#
#   ./release.sh 0.1.0            -> dist/MiniOBS-0.1.0.dmg
#
...
DIST=dist
STAGE="$DIST/stage"
DMG="$DIST/MiniOBS-$VERSION.dmg"
rm -rf "$STAGE" "$DMG"
mkdir -p "$STAGE"
cp -R MiniOBS.app "$STAGE/"
ln -s /Applications "$STAGE/Applications"

hdiutil create -volname "MiniOBS" -srcfolder "$STAGE" -ov -format UDZO -quiet "$DMG"
rm -rf "$STAGE"

if [ -n "${NOTARY_PROFILE:-}" ] && codesign -dv MiniOBS.app 2>&1 | grep -q "Developer ID"; then
